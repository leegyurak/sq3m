...
class SQLGenerator:
    def __init__(self, database_repository: DatabaseRepository, llm_service: LLMService, history: Optional[MarkdownHistory] = None):
    ...
    def generate_and_execute(self, natural_language: str, tables: list[Table], max_retries: int = 2) -> tuple[SQLQuery, list[dict]]:
        """
        ...
        results = [{"error": f"Query failed after {max_retries + 1} attempts. Last error: {str(last_error)}"}]
    ...
    def _log_to_history(self, natural_language: str, sql_query: SQLQuery, results: list[dict], retry_count: int = 0) -> None:
        ...
        try:
            ...
            asyncio.run(self.history.add_query_and_response(
