...
from ...domain.entities.database import (
    DatabaseConnection, DatabaseSchema, Table, Column, Index, DatabaseType
)
...
class MySQLRepository(DatabaseRepository):
    ...
    def get_tables(self) -> list[Table]:
        ...
        for table_data in table_info:
            ...
            table_comment = table_data['TABLE_COMMENT'] if table_data['TABLE_COMMENT'] else None
    ...
    def _get_table_columns(self, table_name: str) -> list[Column]:
        ...
        for col_data in self.cursor.fetchall():
            column = Column(
                ...
                comment=col_data['COLUMN_COMMENT'] if col_data['COLUMN_COMMENT'] else None
            )
