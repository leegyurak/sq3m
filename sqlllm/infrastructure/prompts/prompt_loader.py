...
class PromptLoader:
    ...
    def load_system_prompt(self, custom_path: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            if custom_path or os.getenv("SYSTEM_PROMPT_PATH") or os.getenv("SYSTEM_PROMPT_FILE"):
                ...
                if language in self.language_prompts and self.language_prompts[language].exists():
                    return self.language_prompts[language].read_text(encoding="utf-8").strip()
                # If preferred language doesn't exist, try English
                elif self.language_prompts["en"].exists():
                    return self.language_prompts["en"].read_text(encoding="utf-8").strip()
                # Last resort: try Korean
                elif self.language_prompts["ko"].exists():
                    return self.language_prompts["ko"].read_text(encoding="utf-8").strip()
            
            raise RuntimeError(f"System prompt file not found at {prompt_path} and no language-specific prompts available")
        except Exception as e:
            raise RuntimeError(f"Failed to load system prompt from {prompt_path}: {e}")
    
    def _resolve_prompt_path(self, custom_path: Optional[str] = None) -> Path:
    ...
    def create_custom_prompt_template(self, output_path: str) -> Path:
        ...
        template_content = f"""# Custom System Prompt for SQLLLM
# 
...
"""
    ...
    def get_language_prompt_info(self) -> dict:
