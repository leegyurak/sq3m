...
class CLI:
    ...
    def run(self):
        self.console.print(Panel.fit("🤖 SQLLLM - AI-Powered Database Query Assistant", style="bold blue"))
    ...
    def _setup_database(self) -> bool:
        ...
        self.console.print(f"Connecting to {connection.database_type.value} database...", style="dim")
    ...
    def _prompt_database_connection(self) -> DatabaseConnection | None:
        ...
        for i, db_type in enumerate(db_types, 1):
            self.console.print(f"  {i}. {db_type.upper()}")
    ...
    def _display_table_purposes(self, purposes: dict):
    ...
    def _start_interactive_session(self):
        self.console.print(Panel.fit("🚀 Interactive Query Session Started", style="bold green"))
        self.console.print("Type your questions in natural language. Type 'quit' or 'exit' to stop.\n")
...
• "Show user details for user ID 123"
    ...
    def _process_natural_language_query(self, query: str):
        ...
        try:
            ...
            if sql_query.sql:
                self.console.print("\n📝 Generated SQL:")
                syntax = Syntax(sql_query.sql, "sql", theme="monokai", line_numbers=True)
                ...
                if sql_query.confidence:
                    confidence_color = "green" if sql_query.confidence > 0.7 else "yellow" if sql_query.confidence > 0.4 else "red"
                    self.console.print(f"🎯 Confidence: {sql_query.confidence:.1%}", style=confidence_color)
    ...
    def _display_query_results(self, results: list):
        if not results:
            return
        ...
        if results:
            ...
            for row in results[:20]:
                result_table.add_row(*[str(value) if value is not None else "NULL" for value in row.values()])
