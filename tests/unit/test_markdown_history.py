...
@pytest.fixture
def history_service(temp_history_dir, cleanup_chat_sessions):
...
class TestMarkdownHistory:
    ...
    def test_get_session_filename(self, history_service):
        """Test session filename generation."""
        with patch('sqlllm.infrastructure.history.markdown_history.datetime') as mock_datetime:
    ...
    def test_get_session_filename_special_chars(self, history_service):
        """Test session filename generation with special characters."""
        with patch('sqlllm.infrastructure.history.markdown_history.datetime') as mock_datetime:
            mock_datetime.now.return_value = datetime(2024, 1, 1, 10, 30, 45)
            
            filename = history_service._get_session_filename("test/session:with*special")
            
            # Special characters should be removed
            assert filename == "20240101_103045_testsessionwithspecial.md"

    @pytest.mark.asyncio
    async def test_start_new_session(self, history_service):
    ...
    @pytest.mark.asyncio
    async def test_add_user_query(self, history_service):
    ...
    @pytest.mark.asyncio
    async def test_add_user_query_auto_start_session(self, history_service):
    ...
    @pytest.mark.asyncio
    async def test_add_sql_response(self, history_service):
    ...
    @pytest.mark.asyncio
    async def test_add_sql_response_no_execution_result(self, history_service):
    ...
    @pytest.mark.asyncio
    async def test_add_analysis_result(self, history_service):
    ...
    @pytest.mark.asyncio
    async def test_add_error(self, history_service):
    ...
    @pytest.mark.asyncio
    async def test_add_custom_entry(self, history_service):
    ...
    @pytest.mark.asyncio
    async def test_close_session(self, history_service):
    ...
    @pytest.mark.asyncio
    async def test_close_session_no_active_session(self, history_service):
        """Test closing session when no active session."""
        result = await history_service.close_session()
        assert result is None

    def test_list_sessions(self, history_service):
    ...
    @pytest.mark.asyncio
    async def test_get_recent_sessions(self, history_service):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_get_recent_sessions_with_error(self, history_service):
        ...
        with patch('sqlllm.infrastructure.history.markdown_history.aiofiles.open', side_effect=IOError("Mock error")):
    ...
    def test_get_current_session_info_active(self, history_service):
    ...
    def test_get_current_session_info_inactive(self, history_service):
