...
from sqlllm.domain.entities.database import Table, Column, DatabaseSchema, DatabaseType
...
def mock_openai_client():
    ...
    chat_response.choices[0].message.content = "This table stores user account information."
...
def sample_database_schema():
    ...
    users_table = Table(
        ...
        columns=[
            ...
            Column(name="email", data_type="VARCHAR", is_nullable=False, is_primary_key=False),
            Column(name="password_hash", data_type="VARCHAR", is_nullable=False, is_primary_key=False),
            Column(name="created_at", data_type="TIMESTAMP", is_nullable=False, is_primary_key=False),
            Column(name="updated_at", data_type="TIMESTAMP", is_nullable=True, is_primary_key=False)
        ],
        indexes=[],
    ...
    orders_table = Table(
        ...
        columns=[
            Column(name="id", data_type="INTEGER", is_nullable=False, is_primary_key=True),
            Column(name="user_id", data_type="INTEGER", is_nullable=False, is_primary_key=False),
            Column(name="total_amount", data_type="DECIMAL", is_nullable=False, is_primary_key=False),
            Column(name="status", data_type="VARCHAR", is_nullable=False, is_primary_key=False),
            Column(name="created_at", data_type="TIMESTAMP", is_nullable=False, is_primary_key=False)
        ],
        indexes=[],
...
@pytest.fixture
def mock_database_repository(sample_database_schema):
    """Mock database repository."""
    repo = Mock()
    repo.get_schema.return_value = sample_database_schema
    repo.get_table_sample_rows.side_effect = lambda table_name, limit: [
...
class TestFullWorkflow:
    ...
    async def test_complete_analysis_workflow(
        ...
        llm_service.async_client.chat.completions.create.return_value = mock_openai_client.chat.completions.create.return_value
    ...
    async def test_rag_integration_workflow(
        ...
        class MockRAGService:
            ...
            def add_schema(self, schema):
                self.metadata = [{"table": table.name, "purpose": table.comment} 
                               for table in schema.tables]
                
            def search_relevant_tables(self, query, top_k=2):
                return [
                    {"table": "users", "relevance_score": 0.9, "purpose": "User accounts"},
                    {"table": "orders", "relevance_score": 0.7, "purpose": "Customer orders"}
                ][:top_k]
    ...
    @pytest.mark.asyncio
    async def test_history_integration_workflow(self, temp_dirs, cleanup_chat_sessions):
    ...
    @pytest.mark.asyncio
    async def test_environment_detection_workflow(self, temp_dirs):
    ...
    async def test_end_to_end_workflow(
        ...
        class MockRAGService:
            ...
            def search_relevant_tables(self, query, top_k=2):
                return [
                    {"table": "users", "relevance_score": 0.9, "purpose": "User accounts"},
                    {"table": "orders", "relevance_score": 0.8, "purpose": "Customer orders"}
                ][:top_k]
